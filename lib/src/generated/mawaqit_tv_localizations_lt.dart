import 'mawaqit_tv_localizations.dart';

// ignore_for_file: type=lint

/// The translations for Lithuanian (`lt`).
class MawaqitTvLocalizationsLt extends MawaqitTvLocalizations {
  MawaqitTvLocalizationsLt([String locale = 'lt']) : super(locale);

  @override
  String get thisIsATestFromIbrahim => 'This is a test from Ibrahim';

  @override
  String get home => 'Pagrindinis';

  @override
  String get share => 'Dalintis';

  @override
  String get about => 'Apie';

  @override
  String get rate => 'Įvertinkite mus';

  @override
  String get languages => 'Kalbos';

  @override
  String get appLang => 'Programėlės kalba';

  @override
  String get descLang => 'Pasirinkite pageidaujamą kalbą';

  @override
  String get hadithLangDesc =>
      'Tai pakeis jūsų pasirinkimą administratoriaus konsolėje, ekrane galite pasirinkti kitą kalbą.';

  @override
  String get whoops => 'Ups!';

  @override
  String get noInternet => 'Nėra interneto ryšio';

  @override
  String get tryAgain => 'Pabandykite dar kartą';

  @override
  String get closeApp => 'Uždaryti programą';

  @override
  String get quit => 'Baigti';

  @override
  String get forceStaging => 'Staging';

  @override
  String get forcePreProduction => 'Pre-production';

  @override
  String get disableStaging => 'Perėjimas prie gamybos';

  @override
  String get environmentSwitchSuccess => 'Environment switched successfully';

  @override
  String get environmentSwitchFailed => 'Failed to switch environment';

  @override
  String get sureCloseApp => 'Ar tikrai norite baigti programą?';

  @override
  String get ok => 'GERAI';

  @override
  String get cancel => 'CANCEL';

  @override
  String get darkMode => 'Tamsusis režimas';

  @override
  String get lightMode => 'Šviesos režimas';

  @override
  String get changeMosque => 'Pakeisti mečetę';

  @override
  String get in1 => 'svetainėje';

  @override
  String get azanIn => 'Azanas po';

  @override
  String get sec => 'Sec';

  @override
  String get online => 'Internete';

  @override
  String get missingMosqueId => 'Trūksta MAWAQIT #ID arba MOSQUE #ID';

  @override
  String mosqueIdIsNotValid(Object mosqueId) {
    return 'Atsiprašome, $mosqueId nėra galiojantis mečetės pažymėjimas.';
  }

  @override
  String get selectMosqueId => 'Įveskite savo mečetės ID';

  @override
  String get mawaqitWelcome => 'Sveiki atvykę į MAWAQIT';

  @override
  String get mawaqitDesc =>
      'Assalamu Alaikom ir Baraka\'Allah fikom už tai, kad pasirinkote MAWAQIT, pirmąjį ir pirmąjį pasaulyje išmaniųjų mečečių tinklą, kurį nuo 2016 m. naudoja milijonai musulmonų visame pasaulyje 85+ šalyse.\n\nSuteikiame jums pažangiausią Išmaniosios mečetės ekraną, pasiekiamą per daugybę įrenginių (mobiliuosius telefonus, išmaniuosius laikrodžius, televizorių ekranus), nerenkant ir nesidalijant jūsų asmeniniais duomenimis.\n\nPrašome paremti šį palaimintą projektą čia : https://donate.mawaqit.net\n\nEsame ne pelno siekianti organizacija, o šis projektas yra \"Waqf fi\'sabili Allah\" (Dedicated endowment).\n\nJūsų aukos užtikrina, kad šis projektas būtų prieinamas bet kam ir bet kur, visiškai NEMOKAMAI, be jokios reklamos ir be jokios mėnesinės prenumeratos.\n\nŠis projektas nebūtų įmanomas be Alacho pagalbos, kuris subūrė talentingų ir aistringų savanorių bendruomenę, dirbančią dieną ir naktį, kad suteiktų jums geriausią įmanomą paslaugą ir naujausią sistemą, kuria galima naudotis 24 valandas per parą, 7 dienas per savaitę.\n\nPrašome apsvarstyti galimybę paaukoti lėšų, kad šis palaimintas projektas tęstųsi. Baraka\'Allah fikom už jūsų nuolatinį pasitikėjimą ir paramą.';

  @override
  String get privacyPolicy => 'Privatumo politika';

  @override
  String get termsOfService => 'Paslaugų teikimo sąlygos';

  @override
  String get installationGuide => 'Įrengimo vadovas';

  @override
  String get drawerTitle => 'MAWAQIT';

  @override
  String get drawerDesc => 'Musulmonų ryšys su mečetėmis';

  @override
  String get backendError =>
      'Atsiprašome, nepavyko prisijungti prie serverio.\nPatikrinkite interneto ryšį arba bandykite vėliau.';

  @override
  String get selectWithMosqueId => 'Pabandykite: 256, tai \"Grande Mosquée de Paris\" ID';

  @override
  String get searchForMosque => 'Kurios mečetės ieškote ? (ID, pavadinimas, miestas, pašto kodas...)';

  @override
  String get searchMosque => 'Mečetės paieška';

  @override
  String get mosqueNameError => 'Įveskite mečetės pavadinimą';

  @override
  String get slugError => 'Ar mečetė nėra galiojantis šliužas';

  @override
  String get doYouKnowMosqueId => 'Ar žinote savo įrenginio ID arba mečetės ID?';

  @override
  String get yes => 'Taip';

  @override
  String get no => 'Ne';

  @override
  String get networkStatus => 'Tinklo būsena';

  @override
  String get mosqueNoMore => 'Daugiau jokių rezultatų';

  @override
  String get mosqueNoResults => 'Rezultatų nėra';

  @override
  String get offline => 'Neprisijungus prie interneto';

  @override
  String get imsak => 'Imsakas';

  @override
  String get jumua => 'Jumua';

  @override
  String get duhr => 'Duhr';

  @override
  String get fajr => 'Fajr';

  @override
  String get asr => 'Asr';

  @override
  String get maghrib => 'Maghrib';

  @override
  String get isha => 'Isha';

  @override
  String get afterAdhanHadithTitle => 'Po adhano Du`aa';

  @override
  String get afterSalahHadith =>
      'Allahumma Rabba hadhihid-da\'wati-ttammati, was-salatil-qa\'imati, ati Muhammadanil-wasilata wal-fadhilata, wab\'athu maqaman mahmuda nilladhi wa \'adtahu [O Allah, šio tobulo kvietimo (Da\'wah) ir nustatytos maldos (As-Salat) Rubbe, suteik Muhammadui Wasilah ir pranašumą ir iškelk jį į pagirtiną padėtį, kurią jam pažadėjai].';

  @override
  String get alIqama => 'Al Iqama';

  @override
  String get alAdhan => 'Al Adanas';

  @override
  String get turnOfPhones => 'Perjunkite telefonus į tylųjį režimą';

  @override
  String get iqamaIn => 'Iqama in';

  @override
  String get iqamaShowClock => 'Rodyti laikrodį ikamos ekrane';

  @override
  String get iqamaShowClockDesc => 'Rodyti dabartinį laiką ir datą ikamos atgalinės atskaitos ekrane';

  @override
  String get alAthkar => 'Al-Athkar';

  @override
  String get azkarList0 =>
      'Astaghfirullāh, Astaghfirullāh, Astaghfirullāh. Allāhumma Antas-Salām, wa minkas-Salām, tabārakta yā Dhal-Jalāli wal-Ikrām. Allāhumma a\'innī \'alā dhikrika wa shukrika wa ḥusni \'ibādatik';

  @override
  String get azkarList1 =>
      'Subḥānallāh, Alḥamdulillāh, Allāhu Akbar (33 marrā). Lā ilāha illallāhu waḥdahu lā sharīka lah, lahul-mulku wa lahul-ḥamd, wa huwa \'alā kulli shay\'in Qadīr';

  @override
  String get azkarList2 =>
      'Bismillāhir-Raḥmānir-Raḥīm. Qul a`ūdhu birabbin-nās. Malikin-nās. \'Ilāhin-nās. Min sharri \'l-waswāsil-khannās. Alladhī yuwaswisu fī ṣudūrin-nās. Minal-jinnati wannās.';

  @override
  String get azkarList3 =>
      'Bismillāhir-Raḥmānir-Raḥīm. Qul a`ūdhu birabbil-falaq. Min sharri mā khalaq. Wa min sharri ghāsiqin idhā waqab. Wa min sharrin-naffāthāti fil-`uqad. Wa min sharri ḥāsidin idhā ḥasad.';

  @override
  String get azkarList4 =>
      'Bismillāhir-Raḥmānir-Raḥīm. Qul huwallāhu aḥad. Allāhuṣ-ṣamad. Lam yalid wa lam yūlad. Wa lam yakun lahu kufuwan aḥad.';

  @override
  String get azkarList5 =>
      'Allāhu lā ilāha illā Huwal-Ḥayyul-Qayyūm, lā ta\'khudhuhu sinatun wa lā nawm, lahu mā fis-samāwāti wa mā fil-arḍ, man dhalladhī yashfa\'u \'indahu illā bi\'idhnih, ya\'lamu mā bayna aydīhim wa mā khalfahum, wa lā yuḥīṭūna bishay\'im-min \'ilmihi illā bimā shā\', wasi\'a Kursiyyuhus-samāwāti wal-arḍ, wa lā ya\'ūduhu ḥifẓuhumā, wa Huwal-\'Aliyyul-\'Aẓīm';

  @override
  String get azkarList6 =>
      'Lā \'ilāha \'illallāh, waḥdahu lā sharīka lah, lahu \'l-mulku wa lahu \'l-ḥamd, wa huwa `alā kulli shay\'in qadīr, Allāhumma lā māni`a limā \'a`tayt, wa lā mu`tiya limā mana`t, wa lā yanfa`u dhal-jaddi minkal-jadd.';

  @override
  String get azkarList7 =>
      'Allāhumma anta Rabbī lā ilāha illā ant, khalaqtanī wa anā `abduk, wa anā `alā `ahdika wa wa`dika mastaṭa`t, a`ūdhu bika min sharri mā ṣana`t, abū\'u laka bi ni`matika `alay, wa abū\'u bidhanbī faghfir lī fa\'innahu lā yaghfirudh-dhunūba illā ant.';

  @override
  String get azkarList8 =>
      'Aṣbaḥnā wa aṣbaḥal-mulku lillāh, walḥamdu lillāh, lā ilāha illallāhu waḥdahu lā sharīka lah, lahul-mulku wa lahul-ḥamd, wa huwa `alā kulli shay\'in Qadīr. Rabbi as\'aluka khayra mā fī hādha \'l-yawmi wa khayra mā ba`dahu wa a`ūdhu bika min sharri mā fī hātha \'l-yawmi wa sharri mā ba`dahu, Rabbi a`ūdhu bika minal-kasali, wa sū\'il-kibar, Rabbi a`ūdhu bika min `adhābin fin-nāri wa `adhābin fil-qabr.';

  @override
  String get azkarList9 =>
      'Allāhumma innī aṣbaḥtu ush-hiduka wa ush-hidu ḥamalata `arshik, wa malā\'ikataka wajamī`a khalqik, annaka antallāhu lā ilāha illā ant, waḥdaka lā sharīka lak, wa anna Muḥammadan `abduka wa rasūluk. (Arba\'a marrāt) [Wa idhā amsā qāla: Allāhumma innī amsaytu...]';

  @override
  String get azkarList10 =>
      'Allāhumma `āfinī fī badanī, Allāhumma `āfinī fī sam`ī, Allāhumma `āfinī fī baṣarī, lā ilāha illā ant. Allāhumma innī a`ūdhu bika mina \'l-kufri, wa \'l-faqr, wa a`ūdhu bika min `adhābi \'l-qabr, lā ilāha illā ant. (Thalātha marrāt)';

  @override
  String get azkarList11 =>
      'Ḥasbiyallāhu lā ilāha illā huwa `alayhi tawakkalt, wa huwa Rabbu \'l-`Arshi \'l-\'Aẓīm. (Sab\'a marrāt)';

  @override
  String get azkarList12 =>
      'Raḍītu billāhi Rabba, wa bil-Islāmi dīna, wa bi-Muḥammadin (ṣallallāhu `alayhi wa sallama) nabiyya. (Thalātha marrāt)';

  @override
  String get azkarList13 =>
      'Lā ilāha illallāh waḥdahu lā sharīka lah, lahu\'l-mulku wa lahu\'l-ḥamd yuḥyī wa yumīt wa huwa `alā kulli shay\'in qadīr. (\'Ashra marrāt)';

  @override
  String get azkarList14 =>
      'Amsaynā wa amsal-mulku lillāh, walḥamdulillāh, wa lā ilāha illallāhu waḥdahu lā sharīka lah, lahul-mulku wa lahul-ḥamd, wa huwa \'alā kulli shay\'in Qadīr. Rabbi as\'aluka khayra mā fī hādhihil-laylah, wa khayra mā ba\'dahā, wa a\'ūdhu bika min sharri mā fī hādhihil-laylah, wa sharri mā ba\'dahā, wa a\'ūdhu bika minal-kasal, wa sū\'il-kibar, wa a\'ūdhu bika min \'adhābin-nār, wa \'adhābil-qabr';

  @override
  String get jumuaaScreenTitle => 'Jumuaa laikas';

  @override
  String get jumuaaHadith =>
      'Pranašas ﷺ (ramybė ir Dievo palaiminimas jam) sakė: \"Kas puikiai atlieka prausimąsi, tada eina į jumua, o paskui klausosi ir tyli, tam atleidžiama tai, kas yra nuo to laiko iki kito penktadienio ir dar trijų dienų, o tas, kuris liečia akmenis, tikrai padarė beprasmybę\"';

  @override
  String get shuruk => 'Shuruk';

  @override
  String get duha => 'Duha';

  @override
  String get duhaTime => 'Duha Time';

  @override
  String get reset => 'Iš naujo nustatyti';

  @override
  String get mosqueNotFoundMessage =>
      'Atsiprašome, jūsų mečetė nebuvo rasta, galbūt jos nėra arba ji laikinai išjungta.';

  @override
  String get noInternetMessage =>
      'Nėra interneto prieigos. Patikrinkite interneto ryšį ir bandykite dar kartą. Ar jūsų \"Wi-Fi\" arba eterneto ryšys prijungtas?';

  @override
  String get error => 'Klaida';

  @override
  String get mosqueErrorMessage =>
      'Mečetės klaida, jei esate mečetės administratorius, susisiekite su mūsų palaikymo komanda, kad išspręstumėte šią problemą';

  @override
  String get muharram => 'Muharramas';

  @override
  String get safar => 'Safar';

  @override
  String get rabiAlawwal => 'Rabi\' al-Awwal';

  @override
  String get rabiAlthani => 'Rabi\' al-Thani';

  @override
  String get jumadaAlula => 'Jumada al-Ula';

  @override
  String get jumadaAlakhirah => 'Jumada al-Akhirah';

  @override
  String get rajab => 'Radžabas';

  @override
  String get shaban => 'Sha`ban';

  @override
  String get ramadan => 'Ramadanas';

  @override
  String get shawwal => 'Shawwal';

  @override
  String get dhuAlqidah => 'Dhu al-Qi\'dah';

  @override
  String get dhuAlhijjah => 'Dhu al-Hidža';

  @override
  String get duaaBetweenSalahAndAdhan =>
      'Anas bin Malikas sakė: Allaho Pasiuntinys ﷺ pasakė: Maldavimas negrįžta tarp kvietimo į maldą ir atsistojimo maldai.';

  @override
  String get salatKhayrMinaNawm => 'Assalatu khayrun mina nawm';

  @override
  String get salatElEid => 'Salat El Eid';

  @override
  String get webView => 'Įjungti senąjį režimą';

  @override
  String get developersHomeScreen => 'Kūrėjo pagrindinis ekranas';

  @override
  String get onlineHome => 'Prisijungę prie interneto';

  @override
  String get prayerTimes => 'Maldos laikai';

  @override
  String get alerts => 'Įspėjimas';

  @override
  String get iqamaaCountDown => 'Iqamaa Count Down';

  @override
  String get afterAdhanHadith => 'Po Adhan Hadith';

  @override
  String get afterSalahAzkar => 'Po Salah Azkar';

  @override
  String get iqama => 'Iqama';

  @override
  String get randomHadith => 'Atsitiktinis Hadis';

  @override
  String get announcement => 'Skelbimai';

  @override
  String get jumuaaLive => 'Jumuaa [Tiesioginė transliacija]';

  @override
  String get showSecondaryScreen => 'Naudoti kaip papildomą ekraną (Skelbimams)';

  @override
  String get normalScreen => 'Naudoti kaip pagrindinį ekraną';

  @override
  String get duaaRemainder => 'Duomenų apie likutį';

  @override
  String get fajrWakeUp => 'Fajr pabudimas';

  @override
  String get changeLanguage => 'Keisti kalbą';

  @override
  String get forceScreen => 'Jėgos ekranas';

  @override
  String get clear => 'Skaidrus';

  @override
  String get changeTheme => 'Keisti temą';

  @override
  String get next => 'Kitas';

  @override
  String get mainScreenOrSecondaryScreen => 'Ekrano vieta';

  @override
  String get mainScreenOrSecondaryScreenEXPLINATION =>
      'Ar norite įrengti šį ekraną pagrindiniame maldos kambaryje (vyrų maldos kambaryje)?';

  @override
  String get mainScreen => 'Pagrindinis ekranas';

  @override
  String get secondaryScreen => 'Antrinis ekranas';

  @override
  String get duaaElEftar => 'Duaa El Eftar';

  @override
  String get announcementOnlyMode => 'Skelbimų režimas';

  @override
  String get normalMode => 'Įprastas režimas ';

  @override
  String get announcementOnlyModeEXPLINATION =>
      'Pasirinkite, ar ekrane visą laiką bus rodomi pranešimai; tai gali būti naudinga, jei ekraną įrengsite, pavyzdžiui, prie įėjimo.';

  @override
  String get duaaElEftarText => 'ذهب الظما وابتلت العروق وثبت الاجر ان شاء الله';

  @override
  String get secondaryScreenExplanation =>
      'Jei tai antrinis maldos kambarys (pavyzdžiui, moterų kambarys arba kitas aukštas), šiame ekrane bus rodoma tiesioginė jumua transliacija.';

  @override
  String get mainScreenExplanation =>
      'Pagrindinėje mečetės patalpoje šiame ekrane nebus rodoma tiesioginė jumua transliacija';

  @override
  String get normalModeExplanation => 'Bus rodomas įprastas ekranas su maldos laiku ir pranešimais.';

  @override
  String get announcementOnlyModeExplanation => 'Bus rodomi skelbimai visą laiką';

  @override
  String get orientation => 'Orientacija';

  @override
  String get selectYourMawaqitTvAppOrientation => 'Pasirinkite savo mawaqit tv programėlės orientaciją';

  @override
  String get deviceDefault => 'Įrenginio numatyta';

  @override
  String get deviceDefaultBTNDescription => 'Mawaqit automatiškai parinks numatytą orientaciją pagal ekrano padėtį';

  @override
  String get portrait => 'Vertikali';

  @override
  String get portraitBTNDescription => 'Vertikali orientacija – rekomenduojama mečetėms su nedidele erdve';

  @override
  String get landscape => 'Horizontali';

  @override
  String get landscapeBTNDescription =>
      'Horizontali orientacija. Pagrindinis mawaqit tv išdėstymas ir rekomenduojamas daugumai mečečių';

  @override
  String get eidMubarak => 'Eid Mubarak';

  @override
  String get takbeerAleidText =>
      'Allahu Akbar, Allahu Akbar, Allahu Akbar, Allahu Akbar, la ilaha illa Allah, Allahu Akbar, Allahu Akbar, Allahu Akbar, wa lillahi al-hamd';

  @override
  String get settings => 'Nustatymai';

  @override
  String get applicationModes => 'Numatytasis paleidimo režimas';

  @override
  String get ifYouAreFacingAnIssueWithTheAppActivateThis =>
      'Jei susiduriate su problema su aktyvia programėle, pasirinkite šią parinktį';

  @override
  String get hijriAdjustments => 'Vietiniai hidžros koregavimai';

  @override
  String get hijriAdjustmentsDescription =>
      'Koreguokite hidžros datą vietiškai savo įrenginyje. Tai nepakeis mečetės nustatymų internete';

  @override
  String get backoffice_default => 'Administratoriaus numatyta';

  @override
  String get recommended => 'Rekomenduojama';

  @override
  String get sabah => 'Sabahas';

  @override
  String get randomHadithLanguage => 'Atsitiktinė hadiso kalba';

  @override
  String get mosqueDefault => 'Iš jūsų internetinės konfigūracijos';

  @override
  String get en => 'Anglų kalba';

  @override
  String get fr => 'Prancūzų kalba';

  @override
  String get ar => 'Arabų kalba';

  @override
  String get tr => 'Turkijos';

  @override
  String get de => 'Vokiečių kalba';

  @override
  String get es => 'Ispanų kalba';

  @override
  String get pt => 'Portugalų';

  @override
  String get nl => 'Olandų kalba';

  @override
  String get ta => 'Tamil';

  @override
  String get fr_ar => 'Prancūzų & arabų';

  @override
  String get en_ar => 'Anglų & arabų';

  @override
  String get de_ar => 'Vokiečių & arabų';

  @override
  String get ta_ar => 'Tamilų & arabų';

  @override
  String get tr_ar => 'Turkų & arabų';

  @override
  String get es_ar => 'Ispanų & arabų';

  @override
  String get pt_ar => 'Portugalų & arabų';

  @override
  String get nl_ar => 'Olandų & arabų';

  @override
  String get connectToChangeHadith => 'Norėdami pakeisti hadiso kalbą, prisijunkite prie interneto.';

  @override
  String get retry => 'Pakartoti';

  @override
  String get reciterLoadError => 'Nepavyko įkelti recitatorių';

  @override
  String get reciterNetworkError => 'Patikrinkite interneto ryšį ir bandykite dar kartą';

  @override
  String get reciterServerError => 'Serveris laikinai nepasiekiamas. Bandykite vėliau';

  @override
  String get reciterTimeoutError => 'Užklausa užtruko per ilgai. Bandykite dar kartą';

  @override
  String get surahLoadError => 'Nepavyko įkelti suros';

  @override
  String get timeSetting => 'Laiko konfigūravimas';

  @override
  String get timeSettingDesc => 'Nustatykite pasirinktą pavadinimą';

  @override
  String get selectedTime => 'Dabar pasirinktas laikas';

  @override
  String get confirmation => 'Patvirtinimas';

  @override
  String get confirmationMessage => 'Ar tikrai norite naudoti įrenginio laiką?';

  @override
  String get useDeviceTime => 'Naudoti įrenginio laiką';

  @override
  String get selectTime => 'Pasirinkti laiką';

  @override
  String get previous => 'Ankstesnis';

  @override
  String get appTimezone => 'Programos laiko juosta';

  @override
  String get descTimezone => 'Pasirinkite savo laiko juostą, kad matytumėte tikslius maldos laikus.';

  @override
  String get appWifi => 'Prisijunkite prie wifi';

  @override
  String get descWifi => 'Prisijunkite prie pasirinkto wifi tinklo';

  @override
  String get searchCountries => 'Ieškoti šalių';

  @override
  String get scanAgain => 'Skenuoti dar kartą';

  @override
  String get noScannedResultsFound => 'Netoliese prieigos taškai nerasti';

  @override
  String get connect => 'Prisijungti';

  @override
  String get wifiPassword => 'Slaptažodis';

  @override
  String get skip => 'Praleisti';

  @override
  String get noSSID => '**Paslėptas SSID**';

  @override
  String get close => 'Uždaryti';

  @override
  String get search => 'Paieška';

  @override
  String get wifiSuccess => 'Sėkmingai prisijungta prie Wifi.';

  @override
  String get wifiFailure => 'Nepavyko prisijungti prie Wifi.';

  @override
  String get timezoneSuccess => 'Laiko juosta nustatyta sėkmingai.';

  @override
  String get timezoneFailure => 'Nepavyko nustatyti laiko juostos.';

  @override
  String get screenLock => 'Ekranas įjungtas/išjungtas';

  @override
  String get screenLockConfig => 'Konfigūruoti ekrano įjungimą/išjungimą';

  @override
  String get screenLockMode => 'Ekrano įjungimo/išjungimo režimas';

  @override
  String get screenLockDesc => 'Įjunkite/išjunkite televizorių prieš ir po kiekvienos maldos, kad taupytumėte energiją';

  @override
  String get screenLockDesc2 => 'Ši funkcija įjungia/išjungia įrenginį prieš ir po kiekvieno maldos adžano';

  @override
  String get before => 'min. prieš kiekvieną maldos laiką';

  @override
  String get after => 'min. po kiekvieno maldos laiko';

  @override
  String get updateAvailable => 'Yra atnaujinimas';

  @override
  String get seeMore => 'Rodyti daugiau';

  @override
  String get whatIsNew => 'Kas nauja';

  @override
  String get update => 'Atnaujinti';

  @override
  String get automaticUpdate => 'Pranešti apie atnaujinimą';

  @override
  String get automaticUpdateDescription =>
      'Įjunkite pranešimus apie atnaujinimus, kad gautumėte naujausias funkcijas ir patobulinimus';

  @override
  String get checkInternetLegacyMode => 'Norėdami naudoti seną režimą, turite prisijungti prie interneto';

  @override
  String get powerOnScreen => 'Įjungti ekraną';

  @override
  String get powerOffScreen => 'Išjungti ekraną';

  @override
  String get deviceSettings => 'Įrenginys';

  @override
  String get later => 'Vėliau';

  @override
  String get downloadQuran => 'Atsisiųsti Koraną';

  @override
  String get quran => 'Koranas';

  @override
  String get askDownloadQuran => 'Ar norite atsisiųsti Koraną?';

  @override
  String get download => 'Atsisiųsti';

  @override
  String get downloadingQuran => 'Korano atsisiuntimas';

  @override
  String get extractingQuran => 'Išskleidžiamas Koranas';

  @override
  String get updatedQuran => 'Koranas atnaujintas';

  @override
  String get quranLatestVersion => 'Koranas yra naujausios versijos';

  @override
  String quranUpdatedVersion(Object version) {
    return 'Atnaujinto Korano versija: $version';
  }

  @override
  String get quranIsUpdated => 'Koranas atnaujintas';

  @override
  String get quranDownloaded => 'Koranas atsisiųstas';

  @override
  String get quranIsAlreadyDownloaded => 'Koranas jau atsisiųstas';

  @override
  String get chooseReciter => 'Pasirinkite skaitytoją';

  @override
  String get reciteType => 'Skaitymo tipas';

  @override
  String get readingMode => 'Noriu skaityti';

  @override
  String get listeningMode => 'Noriu klausytis';

  @override
  String quranReadingPage(int leftPage, int rightPage, int totalPages) {
    return 'Puslapis $leftPage - $rightPage / $totalPages';
  }

  @override
  String quranReadingPagePortrait(int currentPage, int totalPages) {
    return 'Puslapis $currentPage / $totalPages';
  }

  @override
  String get chooseQuranPage => 'Pasirinkite puslapį';

  @override
  String get checkingForUpdates => 'Tikrinti atnaujinimus...';

  @override
  String get chooseQuranType => 'Pasirinkite Koraną';

  @override
  String get hafs => 'Hafs';

  @override
  String get warsh => 'Warsh';

  @override
  String get favorites => 'Mėgstamiausi';

  @override
  String get allReciters => 'Visi skaitytojai';

  @override
  String reciterAddedToFavorites(String name) {
    return 'Skaitytojas $name pridėtas prie mėgstamiausių';
  }

  @override
  String reciterRemovedFromFavorites(String name) {
    return 'Skaitytojas $name pašalintas iš mėgstamiausių';
  }

  @override
  String get continueListening => 'Klausyk toliau';

  @override
  String get noFavoriteReciters => 'Nėra mėgstamų skaitytojų. Pridėkite vieną į sąrašą';

  @override
  String get noReciterSearchResult => 'Pagal jūsų paiešką rezultatų nėra';

  @override
  String get searchForReciter => 'Ieškoti skaitytojo';

  @override
  String get downloadAllSuwarSuccessfully => 'Visas Koranas atsisiųstas';

  @override
  String get noSuwarDownload => 'Nėra naujų surų atsisiuntimui';

  @override
  String get connectDownloadQuran => 'Atsisiųsti galite tik prisijungę prie interneto';

  @override
  String get playInOnlineModeQuran => 'Norėdami paleisti, turite būti prisijungę prie interneto';

  @override
  String get downloaded => 'Atsisiųsta';

  @override
  String switchQuranType(String name) {
    return 'Eikite į $name';
  }

  @override
  String get surahSelector => 'Pasirinkite surą';

  @override
  String get checkForUpdates => 'Patikrinkite, ar yra atnaujinimų';

  @override
  String get checkForNewVersion => 'Patikrinkite, ar yra naujos versijos';

  @override
  String get wouldYouLikeToUpdate => 'Ar norėtumėte atnaujinti programėlę?';

  @override
  String get updateCompleted => 'Atnaujinimas sėkmingai baigtas!';

  @override
  String get noUpdates => 'Nėra atnaujinimų';

  @override
  String get usingLatestVersion => 'Naudojate naujausią versiją.';

  @override
  String get updateCancelled => 'Atnaujinimas atšauktas';

  @override
  String get checkingUpdates => 'Tikrinami atnaujinimai...';

  @override
  String get downloadingUpdate => 'Atsisiunčiamas atnaujinimas...';

  @override
  String get installingUpdate => 'Diegiamas atnaujinimas...';

  @override
  String get updateCompletedSuccessfully => 'Atnaujinimas sėkmingai baigtas';

  @override
  String get updateFailed => 'Atnaujinti nepavyko';

  @override
  String get save => 'Išsaugoti';

  @override
  String get enterRtspUrl => 'Įveskite RTSP arba Youtube Live URL adresą';

  @override
  String get addRtspUrl => 'Pridėkite savo kameros transliacijos URL žemiau';

  @override
  String get enableRtspCamera => 'Įjungti kameros transliaciją';

  @override
  String get rtspCameraSettings => 'Kameros nustatymai';

  @override
  String get invalidRtspUrl => 'Neteisingas URL adresas. Patikrinkite adresą ir bandykite dar kartą.';

  @override
  String get validRtspUrl => 'URL patikrintas ir sėkmingai išsaugotas.';

  @override
  String get rtspCameraSettingTitle => 'Tiesioginė kamera';

  @override
  String get rtspCameraSettingDesc =>
      'Prisijunkite prie savo vietinės kameros ir rodykite Džumu\'a maldos transliaciją per televizorių.';

  @override
  String get rtspCameraSettingScreenDesc =>
      'Jei čia įvesite URL, jūsų ekranas automatiškai persijungs į vaizdo transliaciją, kai ateis Džumu\'a laikas';

  @override
  String get validatingStream => 'Transliacija tikrinama...';

  @override
  String get checkInternetLiveCamera => 'Norėdami nustatyti tiesioginę kamerą, turite prisijungti prie interneto';

  @override
  String get somethingWentWrong => 'Įvyko klaida! Bandykite dar kartą';

  @override
  String get somethingWrong => 'Kažkas nutiko ne taip';

  @override
  String get tryAgainLater => 'Pabandykite vėliau';

  @override
  String get hintTextRtspUrl => 'rtsp://... arba https://youtube.com/live/...';

  @override
  String get checkInternetUpdate => 'Norėdami patikrinti, ar yra naujų atnaujinimų, prisijunkite prie interneto';

  @override
  String appUpdateAvailable(String currentVersion, String updatedVersion) {
    return 'Jūsų programėlės versija yra $currentVersion. Yra naujas atnaujinimas (versija $updatedVersion) su naujausiomis funkcijomis ir patobulinimais.';
  }

  @override
  String quranUpdateDialogContent(String moshafName, String version) {
    return 'Yra naujas $moshafName Korano (versija $version) atnaujinimas.';
  }

  @override
  String get ishaAndFajrOnly => 'Tik Fadžr ir Iša maldos';

  @override
  String get minutesBeforeFajrPrayer => 'minutės iki fadžr maldos laiko';

  @override
  String get minutesAfterIshaPrayer => 'minutės po iša maldos laiko';

  @override
  String get scheduleSaved => 'Jūsų tvarkaraštis išsaugotas.';

  @override
  String get completeAllFields => 'Prieš išsaugodami užpildykite visus laukus.';

  @override
  String get endTimeAfter => 'Pabaigos laikas turi būti po pradžios laiko.';

  @override
  String get scheduleListening => 'Planuojamas klausymas';

  @override
  String get enableScheduling => 'Įjungti planavimą';

  @override
  String get scheduleDesc => 'Įjunkite šią funkciją, kad automatiškai grotumėte Surat nustatytu laiku.';

  @override
  String get startTime => 'Pradžios laikas';

  @override
  String get endTime => 'Pabaigos laikas';

  @override
  String get selectReciter => 'Pasirinkite recituotoją';

  @override
  String get selectMoshaf => 'Pasirinkite Mushaf';

  @override
  String get randomSurahSelection => 'Atsitiktinis Suratos pasirinkimas';

  @override
  String get selectSurah => 'Pasirinkite Suratą';

  @override
  String get initializingAutoReading => 'Inicijavimas vyksta...';

  @override
  String get holdOkToStop => 'Laikykite OK norėdami sustabdyti';

  @override
  String prayerTimeNotification(String salahName, String prayerTime) {
    return '$salahName laikas ($prayerTime) pranešimas';
  }

  @override
  String get scheduleInOnlineMode => 'Prisijunkite prie interneto, kad galėtumėte suplanuoti Korano klausymą';

  @override
  String get duaaBetweenAdhanAndIqamaaTitle => 'Malda (Du\'a) nėra atmetama tarp Adhano ir Iqam\'os';

  @override
  String get duaBetweenAdhanIqamah => 'Maldos (Du\'a) tarp Azano ir Iqamos nėra atmetamos.';

  @override
  String get processingRequest => 'Vykdoma užklausa...';

  @override
  String get loadingStream => 'Transliacija kraunama...';

  @override
  String get rtspUrlHint => 'Įveskite RTSP nuorodą arba YouTube adresą';

  @override
  String get urlManagedByMosqueAdmin => 'URL managed by mosque administrator';

  @override
  String get replaceWorkflowWithStream => 'Automatiškai rodyti kameros transliaciją';

  @override
  String get replaceAppWorkflowWithCameraStream =>
      'Ekranas automatiškai rodys kameros transliaciją, kai kamera pradės įrašinėti; jei sustos, ekranas grįš prie maldos laikų rodymo';

  @override
  String get streamError => 'Įvyko transliacijos klaida';

  @override
  String get finish => 'Užbaigti';

  @override
  String get schedulingAlarms => 'Planuojami įjungimo/išjungimo laikai...';

  @override
  String get alarmsSucessSchedule => 'Įjungimo/išjungimo laikai suplanuoti sėkmingai';

  @override
  String get alarmsScheduleFailure => 'Nepavyko suplanuoti įjungimo/išjungimo laikų';

  @override
  String get prayerTimeNotificationTitle => 'Maldos laiko pranešimai';

  @override
  String get prayerTimeNotificationDesc => 'Gaukite adžaną maldos metu net kai programėlė uždaryta';

  @override
  String get enablePrayerReminders => 'Įjungti maldos priminimus';

  @override
  String get enablePrayerRemindersDesc => 'Veikia automatiškai fone';

  @override
  String get testAITranslation => 'Tai yra bandomasis tekstas patikrinti, ar AI vertimas veikia teisingai';

  @override
  String get testCrowdinCI => 'Test string to verify Crowdin CI workflow on develop';

  @override
  String get quranMode => 'Quran mode';

  @override
  String get quranModeExplanation => 'Display the Quran reading screen, starting from the last read page';

  @override
  String get appDisplayMode => 'Display';

  @override
  String get appDisplayModeExplanation => 'Choose how your screen will display content';

  @override
  String get exitQuranModeTitle => 'Exit Quran Mode';

  @override
  String get exitQuranModeMessage => 'Would you like to return to normal mode?';

  @override
  String get settingsSectionGlobal => 'Bendra';

  @override
  String get hijriDateAdjustment => 'Hidžros datos koregavimas';

  @override
  String get interfaceLanguage => 'Sąsajos kalba';

  @override
  String get launchModeMainPrayer => 'Pagrindiniai maldų laikai';

  @override
  String get launchModeSecondaryPrayer => 'Antriniai maldų laikai';

  @override
  String get timezone => 'Laiko juosta';

  @override
  String get wifi => 'WiFi';
}
