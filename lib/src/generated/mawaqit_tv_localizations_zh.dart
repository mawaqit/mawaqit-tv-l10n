import 'mawaqit_tv_localizations.dart';

// ignore_for_file: type=lint

/// The translations for Chinese (`zh`).
class MawaqitTvLocalizationsZh extends MawaqitTvLocalizations {
  MawaqitTvLocalizationsZh([String locale = 'zh']) : super(locale);

  @override
  String get thisIsATestFromIbrahim => 'This is a test from Ibrahim';

  @override
  String get home => '首页';

  @override
  String get share => '分享';

  @override
  String get about => '关于';

  @override
  String get rate => '评价我们';

  @override
  String get languages => '语言';

  @override
  String get appLang => '应用语言';

  @override
  String get descLang => '请选择您喜欢的语言';

  @override
  String get hadithLangDesc => '这覆盖您在管理控制台中的选择，您可以通过屏幕选择不同的语言';

  @override
  String get whoops => '呜呼!';

  @override
  String get noInternet => '没有互联网连接';

  @override
  String get tryAgain => '再试一次';

  @override
  String get closeApp => '关闭应用程序';

  @override
  String get quit => '辞职';

  @override
  String get forceStaging => 'Staging';

  @override
  String get forcePreProduction => 'Pre-production';

  @override
  String get disableStaging => '转为生产';

  @override
  String get environmentSwitchSuccess => 'Environment switched successfully';

  @override
  String get environmentSwitchFailed => 'Failed to switch environment';

  @override
  String get sureCloseApp => '你确定要退出申请吗？';

  @override
  String get ok => '认可';

  @override
  String get cancel => '取消';

  @override
  String get darkMode => '黑暗模式';

  @override
  String get lightMode => '灯光模式';

  @override
  String get changeMosque => '改变清真寺';

  @override
  String get in1 => '在';

  @override
  String get azanIn => 'Azan 在';

  @override
  String get sec => '秘书';

  @override
  String get online => '在线';

  @override
  String get missingMosqueId => '缺少MAWAQIT #ID或MOSQUE #ID';

  @override
  String mosqueIdIsNotValid(Object mosqueId) {
    return '对不起， $mosqueId ，不是有效的清真寺证件。';
  }

  @override
  String get selectMosqueId => '请输入你的清真寺ID';

  @override
  String get mawaqitWelcome => '欢迎来到MAWAQIT';

  @override
  String get mawaqitDesc =>
      '萨拉姆，感谢你选择MAWAQIT，世界上第一个也是排名第一的智能清真寺网络，自2016年以来被全球85个以上国家的数百万穆斯林所使用。\n\n我们为您提供最先进的智能清真寺显示，可在多种设备（手机、智能手表、电视屏幕）上使用，不收集或分享您的个人数据。\n\n请在这里支持这个有福的项目：https://donate.mawaqit.net\n\n我们是一个非营利组织，这个项目是一个 \"Waqf fi\'sabili Allah\"（专门的捐赠）。\n\n你的捐款使这个项目对任何人、任何地方都是免费的，没有任何广告，也没有每月的订阅费。\n\n如果没有真主的帮助，这个项目是不可能实现的，因为真主把一群有才华和热情的志愿者聚集在一起，他们日夜工作，为你提供最好的服务，以及一个24/7的终端系统。\n\n请考虑捐款，让这个有福的项目继续下去。Baraka\'Allah fikom感谢您的持续信任和支持。';

  @override
  String get privacyPolicy => '隐私政策';

  @override
  String get termsOfService => '服务条款';

  @override
  String get installationGuide => '安装指南';

  @override
  String get drawerTitle => '马瓦吉特';

  @override
  String get drawerDesc => '将穆斯林与清真寺联系起来';

  @override
  String get backendError => '对不起，我们无法连接到服务器。\n请确认互联网连接或稍后再试。';

  @override
  String get selectWithMosqueId => '试试。256，这是 \"巴黎大清真寺 \"的身份证。';

  @override
  String get searchForMosque => '您要找的是哪座清真寺？ (身份证、姓名、城市、邮政编码......)';

  @override
  String get searchMosque => '搜索清真寺';

  @override
  String get mosqueNameError => '输入清真寺名称';

  @override
  String get slugError => '不是一个有效的清真寺蛞蝓';

  @override
  String get doYouKnowMosqueId => '你知道你的安装ID或你的清真寺ID吗？';

  @override
  String get yes => '是';

  @override
  String get no => '没有';

  @override
  String get networkStatus => '网络状态';

  @override
  String get mosqueNoMore => '没有更多的结果';

  @override
  String get mosqueNoResults => '没有结果';

  @override
  String get offline => '离线';

  @override
  String get imsak => 'Imsak';

  @override
  String get jumua => '茱木阿';

  @override
  String get duhr => '杜尔';

  @override
  String get fajr => 'Fajr';

  @override
  String get asr => '阿斯尔';

  @override
  String get maghrib => '昏礼';

  @override
  String get isha => 'Isha';

  @override
  String get afterAdhanHadithTitle => '在adhan Du`aa之后';

  @override
  String get afterSalahHadith =>
      'Allahumma Rabba hadhihid-da\'wati-ttammati, was-salatil-qa\'imati, ati Muhammadanil-wasilata wal-fadhilata, wab\'athu maqaman mahmuda nilladhi wa \'adtahu [O Allah, Rubb of this perfect call (Da\'wah) and of the established prayer (As-Salat) , grant Muhammad the Wasilah and superiority, and raise him up to a praiseworthy position that You have promised him]';

  @override
  String get alIqama => '身份证';

  @override
  String get alAdhan => 'Al Athan';

  @override
  String get turnOfPhones => '请将您的手机调至静音模式';

  @override
  String get iqamaIn => 'Iqama在';

  @override
  String get iqamaShowClock => '在伊卡玛屏幕上显示时钟';

  @override
  String get iqamaShowClockDesc => '在伊卡玛倒计时屏幕上显示当前时间和日期';

  @override
  String get alAthkar => 'Al-Athkar';

  @override
  String get azkarList0 => '阿拉善哉，阿拉善哉，阿拉善哉，阿拉善哉，阿拉善哉，阿拉善哉。';

  @override
  String get azkarList1 =>
      'Subhan Allah wal hamdu lillah wallahu akbar（33次） La ilaha illa Allah, wahdahu la charika lah, lahu elmoulku wa lahu elhamdu, wa hua `ala kulli chay in kadir';

  @override
  String get azkarList2 =>
      'بِسۡمِ ٱللَّهِ ٱلرَّحۡمَٰنِ ٱلرَّحِيمِ قُلۡ أَعُوذُ بِرَبِّ ٱلنَّاسِ ، مَلِكِ ٱلنَّاسِ ، إِلَٰهِ ٱلنَّاسِ ، مِن شَرِّ ٱلۡوَسۡوَاسِ ٱلۡخَنَّاسِ ، ٱلَّذِي يُوَسۡوِسُ فِي صُدُورِ ٱلنَّاسِ ، مِنَ ٱلۡجِنَّةِ وَٱلنَّاس';

  @override
  String get azkarList3 =>
      'بِسۡمِ ٱللَّهِ ٱلرَّحۡمَٰنِ ٱلرَّحِيمِ قُلۡ أَعُوذُ بِرَبِّ ٱلۡفَلَقِ ، مِن شَرِّ مَا خَلَقَ ، وَمِن شَرِّ غَاسِقٍ إِذَا وَقَبَ ، وَمِن شَرِّ ٱلنَّفَّٰثَٰتِ فِي ٱلۡعُقَدِ ، وَمِن شَرِّ حَاسِدٍ إِذَا حَسَدَ';

  @override
  String get azkarList4 =>
      'بِسۡمِ ٱللَّهِ ٱلرَّحۡمَٰنِ ٱلرَّحِيمِ قُلۡ هُوَ ٱللَّهُ أَحَدٌ ، ٱللَّهُ ٱلصَّمَدُ ، لَمۡ يَلِدۡ وَلَمۡ يُولَدۡ ، وَلَمۡ يَكُن لَّهُۥ كُفُوًا أَحَدُۢ';

  @override
  String get azkarList5 =>
      'ٱللَّهُ لَآ إِلَٰهَ إِلَّا هُوَ ٱلۡحَيُّ ٱلۡقَيُّومُۚ لَا تَأۡخُذُهُۥ سِنَةٞ وَلَا نَوۡمٞۚ لَّهُۥ مَا فِي ٱلسَّمَٰوَٰتِ وَمَا فِي ٱلۡأَرۡضِۗ مَن ذَا ٱلَّذِي يَشۡفَعُ عِندَهُۥٓ إِلَّا بِإِذۡنِهِۦۚ يَعۡلَمُ مَا بَيۡنَ أَيۡدِيهِمۡ وَمَا خَلۡفَهُمۡۖ وَلَا يُحِيطُونَ بِشَيۡءٖ مِّنۡ عِلۡمِهِۦٓ إِلَّا بِمَا شَآءَۚ وَسِعَ كُرۡسِيُّهُ ٱلسَّمَٰوَٰتِ وَٱلۡأَرۡضَۖ وَلَا يَ‍ُٔودُهُۥ حِفۡظُهُمَاۚ وَهُوَ ٱلۡعَلِيُّ ٱلۡعَظِيمُ';

  @override
  String get azkarList6 =>
      'La ilaha illa Allah, wahdahu la charika lah, lahu elmulku wa lahu elhamdu, wa hua `ala koulli chayin kadir, Allahumma la mani`a lima a`atayte, wa la mu`atia lima`ate, wa la yanefa`u dhal djaddi mineka eldjad';

  @override
  String get azkarList7 =>
      'اللهم أنت ربي، لا إله إلا أنت، خلقتني وأنا عبدُك, وأنا على عهدِك ووعدِك ما استطعتُ، أعوذ بك من شر ما صنعتُ، أبوءُ لَكَ بنعمتكَ عَلَيَّ، وأبوء بذنبي، فاغفر لي، فإنه لا يغفرُ الذنوب إلا أنت';

  @override
  String get azkarList8 =>
      'أصبحنا وأصبح الملك لله، والحمد لله ولا إله إلا الله وحده لا شريك له، له الملك وله الحمد، وهو على كل شيء قدير، أسألك خير ما في هذا اليوم، وخير ما بعده، وأعوذ بك من شر هذا اليوم، وشر ما بعده، وأعوذ بك من الكسل وسوء الكبر، وأعوذ بك من عذاب النار وعذاب القبر';

  @override
  String get azkarList9 =>
      'اللَّهُمَّ إِنِّي أَصْبَحْتُ أُشْهِدُكَ، وَأُشْهِدُ حَمَلَةَ عَرْشِكَ، وَمَلاَئِكَتِكَ، وَجَمِيعَ خَلْقِكَ، أَنَّكَ أَنْتَ اللَّهُ لَا إِلَهَ إِلاَّ أَنْتَ وَحْدَكَ لاَ شَرِيكَ لَكَ، وَأَنَّ مُحَمَّداً عَبْدُكَ وَرَسُولُكَ |أربعَ مَرَّات|. [ وإذا أمسى قال: اللَّهم إني أمسيت...]';

  @override
  String get azkarList10 =>
      '|اللَّهُمَّ عَافِنِي فِي بَدَنِي، اللَّهُمَّ عَافِنِي فِي سَمْعِي، اللَّهُمَّ عَافِنِي فِي بَصَرِي، لاَ إِلَهَ إِلاَّ أَنْتَ. اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنَ الْكُفْرِ، وَالفَقْرِ، وَأَعُوذُ بِكَ مِنْ عَذَابِ القَبْرِ، لاَ إِلَهَ إِلاَّ أَنْتَ |ثلاثَ مرَّاتٍ';

  @override
  String get azkarList11 =>
      '|حَسْبِيَ اللَّهُ لاَ إِلَهَ إِلاَّ هُوَ عَلَيهِ تَوَكَّلتُ وَهُوَ رَبُّ الْعَرْشِ الْعَظِيمِ |سَبْعَ مَرّاتٍ';

  @override
  String get azkarList12 =>
      '|رَضِيتُ بِاللَّهِ رَبَّاً، وَبِالْإِسْلاَمِ دِيناً، وَبِمُحَمَّدٍ صلى الله عليه وسلم نَبِيّاً |ثلاثَ مرَّاتٍ';

  @override
  String get azkarList13 =>
      '|لاَ إِلَهَ إِلاَّ اللَّهُ وَحْدَهُ لاَ شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ، وَهُوَ عَلَى كُلِّ شَيْءٍ قَدِيرٌ |عشرَ مرَّات';

  @override
  String get azkarList14 =>
      'أصبحنا وأصبح الملك لله، والحمد لله ولا إله إلا الله وحده لا شريك له، له الملك وله الحمد، وهو على كل شيء قدير، أسألك خير ما في هذا اليوم، وخير ما بعده، وأعوذ بك من شر هذا اليوم، وشر ما بعده، وأعوذ بك من الكسل وسوء الكبر، وأعوذ بك من عذاب النار وعذاب القبر';

  @override
  String get jumuaaScreenTitle => 'Jumuaa时间';

  @override
  String get jumuaaHadith =>
      'Assalamu Alaikom, and Baraka\'Allah fikom for choosing MAWAQIT, the World\'s first and #1 Smart Mosque Network, used by millions of Muslims worldwide across 85+ countries since 2016. 我们为您提供最先进的智能清真寺显示，可在多种设备（手机、智能手表、电视屏幕）上使用，不收集或分享您的个人数据。我们是一个非营利组织，这个项目是一个 \"Waqf fi\'sabili Allah\"（专门的捐赠）。请支持这个受祝福的项目，https://donate.mawaqit.net。您的捐款使这个项目对任何人、任何地方都是免费的，没有任何广告，也没有月度订阅费用。如果没有真主的帮助，这个项目是不可能实现的，因为真主把一群有才华和热情的志愿者聚集在一起，日以继夜地为你提供最好的服务，以及一个24/7的高端系统。请考虑捐款，让这个有福的项目继续下去。Baraka\'Allah fikum感谢您的持续信任和支持。';

  @override
  String get shuruk => '舒鲁克';

  @override
  String get duha => 'Duha';

  @override
  String get duhaTime => 'Duha Time';

  @override
  String get reset => '复位';

  @override
  String get mosqueNotFoundMessage => '对不起，没有找到你的清真寺，或者它可能失踪或暂时失效。';

  @override
  String get noInternetMessage => '没有互联网接入。请验证您的互联网连接并重试。你的Wi-Fi或以太网是否已连接？';

  @override
  String get error => '误差';

  @override
  String get mosqueErrorMessage => '清真寺错误，如果你是清真寺管理员，请联系我们的支持以解决这个问题。';

  @override
  String get muharram => '穆哈拉姆';

  @override
  String get safar => '萨法尔';

  @override
  String get rabiAlawwal => '腊月';

  @override
  String get rabiAlthani => '拉比-萨尼（Rabi\' al-Thani';

  @override
  String get jumadaAlula => 'Jumada al-Ula';

  @override
  String get jumadaAlakhirah => 'Jumada al-Akhirah';

  @override
  String get rajab => '拉贾布';

  @override
  String get shaban => '沙班';

  @override
  String get ramadan => '斋月';

  @override
  String get shawwal => '沙瓦勒';

  @override
  String get dhuAlqidah => 'Dhu al-Qi\'dah';

  @override
  String get dhuAlhijjah => '希吉节';

  @override
  String get duaaBetweenSalahAndAdhan => '阿纳斯-本-马利克说。安拉的使者（ﷺ）说。祈求在召唤祈祷和站立祈祷之间不返回。';

  @override
  String get salatKhayrMinaNawm => '萨勒图-哈伊伦-米娜-纳乌姆。';

  @override
  String get salatElEid => '开斋节礼拜';

  @override
  String get webView => '启用遗留模式';

  @override
  String get developersHomeScreen => '开发者的主屏幕';

  @override
  String get onlineHome => '网上主页';

  @override
  String get prayerTimes => '祈祷时间';

  @override
  String get alerts => '警报';

  @override
  String get iqamaaCountDown => 'Iqamaa倒计时';

  @override
  String get afterAdhanHadith => '阿丹圣训后';

  @override
  String get afterSalahAzkar => '在萨拉赫-阿兹卡尔之后';

  @override
  String get iqama => '身份证号：Iqama';

  @override
  String get randomHadith => '随机圣训';

  @override
  String get announcement => '公告';

  @override
  String get jumuaaLive => 'Jumuaa [现场直播]';

  @override
  String get showSecondaryScreen => '作为辅助屏幕使用（用于公告）。';

  @override
  String get normalScreen => '作为主屏幕使用';

  @override
  String get duaaRemainder => 'Duaa 剩余部分';

  @override
  String get fajrWakeUp => '朝圣醒来的时候';

  @override
  String get changeLanguage => '改变语言';

  @override
  String get forceScreen => '力量屏幕';

  @override
  String get clear => '清楚';

  @override
  String get changeTheme => '改变主题';

  @override
  String get next => '下一页';

  @override
  String get mainScreenOrSecondaryScreen => '屏幕位置';

  @override
  String get mainScreenOrSecondaryScreenEXPLINATION => '你想在主祈祷室（男祈祷室）安装这个屏幕吗？';

  @override
  String get mainScreen => '主屏幕';

  @override
  String get secondaryScreen => '二级屏幕';

  @override
  String get duaaElEftar => 'Duaa El Eftar';

  @override
  String get announcementOnlyMode => '公告模式';

  @override
  String get normalMode => '正常模式 ';

  @override
  String get announcementOnlyModeEXPLINATION => '选择你的屏幕是否会一直显示公告，如果你把屏幕安装在入口处，这可能很有用，例如。';

  @override
  String get duaaElEftarText => 'ذهب الظما وابتلت العروق وثبت الاجر ان شاء الله';

  @override
  String get secondaryScreenExplanation => '对于一个次要的祈祷室（例如，妇女室或其他楼层），这个屏幕将显示jumua的现场直播';

  @override
  String get mainScreenExplanation => '对于主要的清真寺房间，这个屏幕将不显示jumua直播。';

  @override
  String get normalModeExplanation => '将显示正常的祈祷时间和公告的屏幕。';

  @override
  String get announcementOnlyModeExplanation => '将显示所有时间的公告';

  @override
  String get orientation => '方向';

  @override
  String get selectYourMawaqitTvAppOrientation => '选择您的 mawaqit tv 应用方向';

  @override
  String get deviceDefault => '设备默认';

  @override
  String get deviceDefaultBTNDescription => 'Mawaqit 将根据屏幕方向自动选择默认方向';

  @override
  String get portrait => '头像';

  @override
  String get portraitBTNDescription => '对于为清真寺建议的空间小的垂直方向';

  @override
  String get landscape => '横屏模式';

  @override
  String get landscapeBTNDescription => '对于水平方向。Mawaqit tv 应用的主要布局，并推荐大多数清真寺的主要布局。';

  @override
  String get eidMubarak => '开斋节';

  @override
  String get takbeerAleidText =>
      'Allahu Akbar, Allahu Akbar, Allahu Akbar, la ilaha illa Allah, Allahu Akbar, Allahu Akbar, wa lillahi al-hamd';

  @override
  String get settings => '设置';

  @override
  String get applicationModes => '默认启动模式';

  @override
  String get ifYouAreFacingAnIssueWithTheAppActivateThis => '如果你遇到了应用程序的问题，可以选择这个选项。';

  @override
  String get hijriAdjustments => '本地Hijri调整';

  @override
  String get hijriAdjustmentsDescription => '在您的设备上调整本地的 hirri 日期。这不会影响在线清真寺设置';

  @override
  String get backoffice_default => '后台默认值';

  @override
  String get recommended => '推荐的';

  @override
  String get sabah => 'Sabah';

  @override
  String get randomHadithLanguage => '随机hadith语言';

  @override
  String get mosqueDefault => '从您的在线配置';

  @override
  String get en => '英语';

  @override
  String get fr => '法国';

  @override
  String get ar => '阿拉伯语';

  @override
  String get tr => '土耳其';

  @override
  String get de => '德国';

  @override
  String get es => '西班牙';

  @override
  String get pt => '葡萄牙文';

  @override
  String get nl => '荷兰语';

  @override
  String get ta => 'Tamil';

  @override
  String get fr_ar => '法语和阿拉伯语';

  @override
  String get en_ar => '英语和阿拉伯语';

  @override
  String get de_ar => '德语和阿拉伯语';

  @override
  String get ta_ar => 'Tamil & Arabic';

  @override
  String get tr_ar => '土耳其语和阿拉伯语';

  @override
  String get es_ar => '西班牙语和阿拉伯语';

  @override
  String get pt_ar => '葡萄牙语和阿拉伯语';

  @override
  String get nl_ar => '荷兰语和阿拉伯语';

  @override
  String get connectToChangeHadith => '请连接到互联网以更改hadith语言。';

  @override
  String get retry => '重试';

  @override
  String get reciterLoadError => '无法加载监听器';

  @override
  String get reciterNetworkError => '请检查您的网络连接并重试';

  @override
  String get reciterServerError => '服务器暂时不可用。请稍后再试';

  @override
  String get reciterTimeoutError => '请求超时。请重试';

  @override
  String get surahLoadError => '无法加载子项';

  @override
  String get timeSetting => '配置时间';

  @override
  String get timeSettingDesc => '设置自定义名称';

  @override
  String get selectedTime => '当前选择的时间';

  @override
  String get confirmation => '确认';

  @override
  String get confirmationMessage => '您确定要使用设备时间吗？';

  @override
  String get useDeviceTime => '使用设备时间';

  @override
  String get selectTime => '选择时间';

  @override
  String get previous => '上一个';

  @override
  String get appTimezone => 'App Timezone';

  @override
  String get descTimezone => '选择您的时区以获得准确的祈祷时间。';

  @override
  String get appWifi => '连接到 wifi';

  @override
  String get descWifi => '请连接到您喜欢的无线网络';

  @override
  String get searchCountries => '搜索国家';

  @override
  String get scanAgain => '再次扫描';

  @override
  String get noScannedResultsFound => '没有找到接近的接入点';

  @override
  String get connect => '连接';

  @override
  String get wifiPassword => '密码';

  @override
  String get skip => '跳过';

  @override
  String get noSSID => '**Hidden SSID**';

  @override
  String get close => '关闭';

  @override
  String get search => '搜索';

  @override
  String get wifiSuccess => '连接到 Wifi 成功。';

  @override
  String get wifiFailure => '连接到 Wifi 失败。';

  @override
  String get timezoneSuccess => '时区设置成功。';

  @override
  String get timezoneFailure => '设置时区失败。';

  @override
  String get screenLock => '屏幕开/关';

  @override
  String get screenLockConfig => '配置屏幕开启/关闭';

  @override
  String get screenLockMode => '屏幕开关模式';

  @override
  String get screenLockDesc => '在每次祈祷前后打开/关闭电视以节省能源';

  @override
  String get screenLockDesc2 => '这个功能在每个祈祷者到场前后开启/关闭设备';

  @override
  String get before => '每次祈祷前的分钟';

  @override
  String get after => '每次祈祷后的分钟';

  @override
  String get updateAvailable => '可用更新';

  @override
  String get seeMore => '查看更多';

  @override
  String get whatIsNew => '新功能';

  @override
  String get update => '更新';

  @override
  String get automaticUpdate => '通知更新';

  @override
  String get automaticUpdateDescription => '启用通知更新以接收最新功能和改进';

  @override
  String get checkInternetLegacyMode => '您必须连接到互联网才能使用旧版模式';

  @override
  String get powerOnScreen => '屏幕上的功率';

  @override
  String get powerOffScreen => '关闭屏幕';

  @override
  String get deviceSettings => '设备';

  @override
  String get later => '稍后再说';

  @override
  String get downloadQuran => '下载Quran';

  @override
  String get quran => '古兰';

  @override
  String get askDownloadQuran => '您想要下载《古兰经》吗？';

  @override
  String get download => '下载';

  @override
  String get downloadingQuran => '正在下载 Quran';

  @override
  String get extractingQuran => '正在提取Quran';

  @override
  String get updatedQuran => 'Quran 已更新';

  @override
  String get quranLatestVersion => 'Quran 是最新的';

  @override
  String quranUpdatedVersion(Object version) {
    return '古兰经更新版本是：$version';
  }

  @override
  String get quranIsUpdated => 'Quran 已更新';

  @override
  String get quranDownloaded => 'Quran 已下载';

  @override
  String get quranIsAlreadyDownloaded => 'Quran 已经下载';

  @override
  String get chooseReciter => '选择接收者';

  @override
  String get reciteType => 'Recite Type';

  @override
  String get readingMode => '我想阅读一下。';

  @override
  String get listeningMode => '我想要收听的';

  @override
  String quranReadingPage(int leftPage, int rightPage, int totalPages) {
    return '$leftPage - $rightPage / $totalPages 页';
  }

  @override
  String quranReadingPagePortrait(int currentPage, int totalPages) {
    return '$currentPage / $totalPages 页';
  }

  @override
  String get chooseQuranPage => '选择页面';

  @override
  String get checkingForUpdates => '正在检查更新...';

  @override
  String get chooseQuranType => '选择quran';

  @override
  String get hafs => '哈夫斯';

  @override
  String get warsh => '瓦尔什文';

  @override
  String get favorites => '收藏';

  @override
  String get allReciters => '所有Reciter';

  @override
  String reciterAddedToFavorites(String name) {
    return '朗读者 $name 已添加到收藏夹';
  }

  @override
  String reciterRemovedFromFavorites(String name) {
    return '朗读者 $name 已从收藏夹移除';
  }

  @override
  String get continueListening => '继续收听';

  @override
  String get noFavoriteReciters => '没有收藏的最近版。请尝试在列表中添加一个';

  @override
  String get noReciterSearchResult => '没有找到您搜索的结果';

  @override
  String get searchForReciter => '搜寻一名累计者';

  @override
  String get downloadAllSuwarSuccessfully => '整个quran已下载';

  @override
  String get noSuwarDownload => '没有要下载的新的 suwars';

  @override
  String get connectDownloadQuran => '请连接到互联网下载';

  @override
  String get playInOnlineModeQuran => '请连接到网络以播放';

  @override
  String get downloaded => '已下载';

  @override
  String switchQuranType(String name) {
    return '转到 $name';
  }

  @override
  String get surahSelector => 'Select Surah';

  @override
  String get checkForUpdates => '检查更新';

  @override
  String get checkForNewVersion => '检查新版本是否可用';

  @override
  String get wouldYouLikeToUpdate => '您想要更新应用程序吗？';

  @override
  String get updateCompleted => '更新成功！';

  @override
  String get noUpdates => '无更新';

  @override
  String get usingLatestVersion => '您正在使用最新版本。';

  @override
  String get updateCancelled => '更新已取消';

  @override
  String get checkingUpdates => '正在检查更新...';

  @override
  String get downloadingUpdate => '正在下载更新...';

  @override
  String get installingUpdate => '正在安装更新...';

  @override
  String get updateCompletedSuccessfully => '更新成功完成';

  @override
  String get updateFailed => '更新失败';

  @override
  String get save => '保存';

  @override
  String get enterRtspUrl => '输入 RTSP 或 Youtube Live URL';

  @override
  String get addRtspUrl => '在下方添加您的相机流 URL';

  @override
  String get enableRtspCamera => '启用相机流';

  @override
  String get rtspCameraSettings => '相机设置';

  @override
  String get invalidRtspUrl => '无效的 URL。请检查URL，然后重试。';

  @override
  String get validRtspUrl => 'URL验证并保存成功。';

  @override
  String get rtspCameraSettingTitle => '直播摄像头';

  @override
  String get rtspCameraSettingDesc => '连接到您的本地摄像头并在电视屏幕上显示跳跃祈祷流。';

  @override
  String get rtspCameraSettingScreenDesc => '如果您在这里输入一个URL，当Jumua时间到达时，您的屏幕将自动切换到视频流';

  @override
  String get validatingStream => '正在验证流...';

  @override
  String get checkInternetLiveCamera => '您必须连接到互联网以设置实时摄像头';

  @override
  String get somethingWentWrong => '出错了！请重试';

  @override
  String get somethingWrong => '出了问题';

  @override
  String get tryAgainLater => '请稍后再试';

  @override
  String get hintTextRtspUrl => 'rtsp://... 或 https://youtube.com/live/...';

  @override
  String get checkInternetUpdate => '您必须连接到互联网以检查新的更新';

  @override
  String appUpdateAvailable(String currentVersion, String updatedVersion) {
    return '您的应用当前为版本 $currentVersion。发现新更新（版本 $updatedVersion），带来最新功能和改进。';
  }

  @override
  String quranUpdateDialogContent(String moshafName, String version) {
    return '$moshafName 古兰经的新更新（版本 $version）已可用。';
  }

  @override
  String get ishaAndFajrOnly => '仅Fajr和Isha prayers';

  @override
  String get minutesBeforeFajrPrayer => '最佳祷告前几分钟';

  @override
  String get minutesAfterIshaPrayer => '分钟以内祈祷时间';

  @override
  String get scheduleSaved => '您的计划已保存。';

  @override
  String get completeAllFields => '请在保存前完成所有字段。';

  @override
  String get endTimeAfter => '结束时间必须在开始时间之后。';

  @override
  String get scheduleListening => '预定侦听中';

  @override
  String get enableScheduling => '启用计划';

  @override
  String get scheduleDesc => '启用此功能可在预定时间自动播放 Surah';

  @override
  String get startTime => '开始时间';

  @override
  String get endTime => '结束时间';

  @override
  String get selectReciter => '选择接收者';

  @override
  String get selectMoshaf => '选择一个 Mushaf';

  @override
  String get randomSurahSelection => '随机Surah选择';

  @override
  String get selectSurah => '选择一个 Surah';

  @override
  String get initializingAutoReading => '正在初始化...';

  @override
  String get holdOkToStop => '长按OK停止';

  @override
  String prayerTimeNotification(String salahName, String prayerTime) {
    return '$salahName 时间 ($prayerTime) 通知';
  }

  @override
  String get scheduleInOnlineMode => '请连接到互联网来排定Quran监听时间';

  @override
  String get duaaBetweenAdhanAndIqamaaTitle => 'Adhan 和 Iqamah 之间没有拒绝供应(杜阿)';

  @override
  String get duaBetweenAdhanIqamah => 'Adhan和Iqamah之间没有拒绝供应(杜阿)。';

  @override
  String get processingRequest => '正在处理请求...';

  @override
  String get loadingStream => '加载流...';

  @override
  String get rtspUrlHint => '输入 RTSP URL 或 YouTube 链接';

  @override
  String get urlManagedByMosqueAdmin => 'URL managed by mosque administrator';

  @override
  String get replaceWorkflowWithStream => '自动显示相机流';

  @override
  String get replaceAppWorkflowWithCameraStream => '当摄像头开始录制时，屏幕将自动显示相机流； 如果它停止，屏幕返回祈祷时间';

  @override
  String get streamError => '流媒体时出错';

  @override
  String get finish => '完成';

  @override
  String get schedulingAlarms => '正在计划/关闭时间...';

  @override
  String get alarmsSucessSchedule => '打开/关闭触发器已成功计划';

  @override
  String get alarmsScheduleFailure => '未能安排打开/关闭触发器';

  @override
  String get prayerTimeNotificationTitle => '暂停时间通知';

  @override
  String get prayerTimeNotificationDesc => '即使应用程序关闭，也可以在祈祷时获得adhan';

  @override
  String get enablePrayerReminders => '启用暂停提醒';

  @override
  String get enablePrayerRemindersDesc => '后台自动工作';

  @override
  String get testAITranslation => '这是验证AI 翻译正常工作的测试字符串';

  @override
  String get testCrowdinCI => 'Test string to verify Crowdin CI workflow on develop';

  @override
  String get quranMode => 'Quran mode';

  @override
  String get quranModeExplanation => 'Display the Quran reading screen, starting from the last read page';

  @override
  String get appDisplayMode => 'Display';

  @override
  String get appDisplayModeExplanation => 'Choose how your screen will display content';

  @override
  String get exitQuranModeTitle => 'Exit Quran Mode';

  @override
  String get exitQuranModeMessage => 'Would you like to return to normal mode?';

  @override
  String get settingsSectionGlobal => '全局';

  @override
  String get hijriDateAdjustment => '伊斯兰历日期调整';

  @override
  String get interfaceLanguage => '界面语言';

  @override
  String get launchModeMainPrayer => '主要礼拜时间';

  @override
  String get launchModeSecondaryPrayer => '次要礼拜时间';

  @override
  String get timezone => '时区';

  @override
  String get wifi => 'WiFi';
}
