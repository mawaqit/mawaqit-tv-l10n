...
def newest_ai_translation(string_id, lang):
    ...
    return max(ai, key=lambda t: t["createdAt"])["id"] if ai else None
    ...
    for lang in langs:
        ...
        for t in top_translations(file, lang):
            if t["stringId"] in approved or "translationId" not in t:
                continue
            tid = t["translationId"] if t.get("provider") == "ai" else newest_ai_translation(t["stringId"], lang)
            if tid:
